...
$insertCustomer = $db->prepare( "INSERT INTO `customers` (`name`,`website`,`hold`) VALUES(:custName,:website,:hold)" );
...
foreach( $csv as $customer ) {
    ...
    $hold = str_replace( '"' , "" , $customer[16] );


    // Exclude customers with no names
    if( ! empty( $name ) ) {
        $insertCustomer->execute( [ ':custName' => $name , ':website' => $website , ':hold' => $hold ] );
